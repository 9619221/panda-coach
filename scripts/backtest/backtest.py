...
def open_trade(state: State, ts: pd.Timestamp, sig: Signal, inst: str) -> None:
    ...
    if state.weekly_halve_until is not None and ts < state.weekly_halve_until:
        risk_u *= 0.5
    risk_pct = abs(sig.entry - sig.stop) / sig.entry
    if risk_pct <= 0:
        return
    size_usdt = risk_u / risk_pct
    qty_coin = size_usdt / sig.entry
...
def close_trade(state: State, ts: pd.Timestamp, price: float, reason: str, inst: str) -> None:
    t = state.open_trade
    if t is None:
        return
    if t.direction == "long":
        gross = (price - t.entry) * t.qty_coin
    else:
        gross = (t.entry - price) * t.qty_coin
    fee = (t.qty_coin * price) * FEE_RATE
    pnl = gross - fee
    t.close_ts = ts
    t.close_price = price
...
def step_open_position(state: State, ts: pd.Timestamp, high: float, low: float, inst: str) -> None:
    ...
    if hit_sl and hit_tp:
        close_trade(state, ts, t.stop, "sl", inst)
    elif hit_sl:
        close_trade(state, ts, t.stop, "sl", inst)
    elif hit_tp:
        close_trade(state, ts, t.take_profit, "tp", inst)


def run_backtest(inst: str, df15: pd.DataFrame, df1h: pd.DataFrame) -> tuple[State, pd.DataFrame]:
